...
namespace j::rendering::vulkan {
  inline namespace uniform_buffers {
    copy_to_uniform_buffer_operation::copy_to_uniform_buffer_operation(
    ...
    void copy_to_uniform_buffer_operation::execute(command_context & context, state::reserved_resources_t &) const {
      uniform_data_source_manager::instance.copy_to(*context.render_context, m_target, m_source_key);
    }

    strings::string copy_to_uniform_buffer_operation::name() const {
      return "Copy to uniform buffer";
    }
  }
}
